...
import {each_bade_count, each_citation_count} from "../configs/config3";
...
class Cluster extends Component {
   ...
   async  addToSearchEngine(){
      ...
      //alert(knowledgebasePostParaAPI+'?sid='+this.state.sessionid+'?ms='+this.state.membersince+'&stid='+sessionStorage.getItem('studentId'))
       ...
       // citation count
       await axios.get(each_citation_count+'?name='+this.state.paragraph)
           .then(response => {
               //alert(response.data.citation)
               this.setState({citation_cnt: response.data.citation})
       ...
       if(this.state.searchretrieve.length===0){
           ...
           await axios.post(searchAPI, {
               ...
               all_citations:this.state.citation_cnt,
